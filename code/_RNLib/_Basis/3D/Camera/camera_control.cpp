...
void CCamera::ProcessState(const PROCESS process) {
	...
	case STATE::GRAB_AIR_MOUSE: {
		...
		case PROCESS::UPDATE: {
			...
			// ImGuiのウィンドウにフォーカスが当たっている時、処理を終了
			if (ImGui_GetWindowFocused())
				return;
			
			// [[[ カーソルの移動量に応じて回転させる ]]]
			if (RNLib::Input()->MousePress(CInput::MOUSEBUTTON_RIGHT)) {
...

			{
				...
				// [[[ カーソルの移動量に応じて移動させる ]]]
				if (RNLib::Input()->MousePress(CInput::MOUSEBUTTON_LEFT)) {
				...
				if (RNLib::Input()->WheelSpin() == CInput::WHEELSPIN_FRONT) {
				...
				else if (RNLib::Input()->WheelSpin() == CInput::WHEELSPIN_BACK) {
					float XZPlaneRate = 1.0f - fabsf(rotXRate);
