...
void CText2D::Set(
	...
	for (int nCntDigit = 0; nCntDigit < nStrLen; nCntDigit++) {
		...
		// 位置と向きをマトリックス変換
		D3DXMATRIX baseMtx = CMatrix::ConvPosRotToMtx(pos, rot);
		D3DXMATRIX txtMtx = CMatrix::ConvPosRotToMtx(setPos, INITD3DXVECTOR3);

		// 基準マトリックスとテキストマトリックスを掛け合わせ、結果マトリックスを求める
		D3DXMATRIX resultMtx = CMatrix::MultiplyMtx(baseMtx, txtMtx);

		// ポリゴン2Dを設定
		/*RNLib::Polygon2D().Set(
			fScaleX,
			fScaleY,
			CMatrix::ConvMatrixToPos(resultMtx),
